...
function handleDeviceUpdate(device) {
    ...
    // Update Status display with pulse animation if changed
    if (valBrightness.innerText !== device.current_brightness + '%') {
        valBrightness.innerText = device.current_brightness + '%';
        valBrightness.classList.remove('pulse');
        void valBrightness.offsetWidth; // trigger reflow
...
modeBtns.forEach(btn => {
    btn.addEventListener('click', async () => {
        ...
        if (navigator.vibrate) navigator.vibrate([10, 30, 10]); // Haptic click feel
        
        // Optimistic UI Update (Instant response)
        handleDeviceUpdate({ 
            mode: selectedMode, 
            manual_brightness: brightnessSlider.value,
            current_brightness: valBrightness.innerText.replace('%',''),
            current_lux: valLux.innerText
        });
